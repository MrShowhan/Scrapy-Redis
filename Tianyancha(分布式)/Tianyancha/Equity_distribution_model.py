...
class GetData():
    '''
    此模块的代码是直接引用于 网址：’https://www.cnblogs.com/triangle959/p/js_tianyancha.html‘作者：上周的bug还没搞好 。如有侵权，请及时联系删除 QQ：475641767
    '''
    ...
    def getfnstr(self, data):
        fnstr = ""
        for i in data.split(','):
            fnstr += chr(int(i))
        return fnstr

    def getSogo(self, default_list, id):
        r = str(ord(id[0]))
        return default_list[int(r[1])]

    def getfxckStr(self, fxck, window_sogo):
        fxckStr = ""
        for i in fxck.split(','):
    ...
    def get_json(self):
        ...
        res1 = self.s.get(
            "https://capi.tianyancha.com/cloud-equity-provider/v4/qq/name.json?id={}?random={}".format(self.id, str(
                int(time.time() * 1000))), headers=self.headers)
        data_dict = json.loads(res1.content)["data"]

        # 调用加密函数,获取cloud_token 以及cloud_utm
        fnstr = self.getfnstr(data_dict.get('v'))
        cookie_token = re.search('cookie=\'cloud_token\=(.*?)\;', fnstr).group(1)
        wtf_return = re.search('return\'(.*?)\'', fnstr).group(1)
        window_sogo = self.getSogo(self.default_list, self.id)
        # cloud_utm
        fxckStr = self.getfxckStr(wtf_return, window_sogo)
        self.headers["Cookie"] = self.headers[
                                     "Cookie"] + " cloud_utm=" + fxckStr + "; cloud_token=" + cookie_token + ';'
        res2 = self.s.get('https://capi.tianyancha.com/cloud-equity-provider/v4/equity/indexnode.json?id={}'.format(self.id),
